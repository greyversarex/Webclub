import { useCallback, useEffect, useMemo, useRef, useState, memo, ComponentType } from 'react';
...
export const LogoLoop = memo(({
  ...
  const renderLogoItem = useCallback((item: LogoItem, key: string) => {
    ...
    const content = isNode
      ...
      : <img src={(item as ImageLogoItem).src} srcSet={(item as ImageLogoItem).srcSet} sizes={(item as ImageLogoItem).sizes}
          width={(item as ImageLogoItem).width} height={(item as ImageLogoItem).height}
          alt={(item as ImageLogoItem).alt ?? ''} title={(item as ImageLogoItem).title}
          loading="lazy" decoding="async" draggable={false} />;
    const itemAriaLabel = isNode ? ((item as NodeLogoItem).ariaLabel ?? (item as NodeLogoItem).title) : ((item as ImageLogoItem).alt ?? (item as ImageLogoItem).title);
